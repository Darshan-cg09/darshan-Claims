...
claim.attachments = attachments.map(att => ({
  name: att.file_name,
  url: `http://13.48.27.96:6010/uploads/${encodeURIComponent(att.file_path)}`,
  size: att.file_size
}));
    }
...
app.get('/api/claims/:id', async (req, res) => {
  ...
  try {
    ...
    claim.attachments = attachments.map(att => ({
      name: att.file_name,
      url: `http://13.60.87.153:6010/${att.file_path}`,
      size: att.file_size
    }));
...
app.post('/api/claims', upload.array('attachments'), async (req, res) => {
  ...
  try {
    ...
    claim.attachments = attachments.map(att => ({
      name: att.file_name,
      url: `http://13.60.87.153:6010/${path.basename(att.file_path)}`,
      size: att.file_size
    }));
...
app.listen(port, '0.0.0.0', async () => {
  ...
  console.log(`Server running on http://13.60.87.153:${port}`);
